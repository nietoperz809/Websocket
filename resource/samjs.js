            ...
            for (var rulePos = pre.length - 1; rulePos > -1; rulePos--) {
                ...
                if (!flags(ruleByte, FLAG_ALPHA_OR_QUOT)) {
                    if (!{
                            ...
                            ' ': function () {
                                return !flagsAt(text, --pos, FLAG_ALPHA_OR_QUOT);
                            },
                            // '#' - previous char must be a vowel or Y.
                            '#': function () {
                                return flagsAt(text, --pos, FLAG_VOWEL_OR_Y);
                            },
                            // '.' - unknown?
                            '.': function () {
                                return flagsAt(text, --pos, FLAG_0X08);
                            },
                            // '&' - previous char must be a dipthong or previous chars must be 'CH' or 'SH'
                            '&': function () {
                                return (flagsAt(text, --pos, FLAG_DIPTHONG)) || (isOneOf(text.substr(--pos, 2), ['CH', 'SH']));
                            },
                            ...
                            '^': function () {
                                return flagsAt(text, --pos, FLAG_CONSONANT);
                            },
                            // '+' - previous char must be either 'E', 'I' or 'Y'.
                            '+': function () {
                                return isOneOf(text[--pos], EIY);
                            },
            ...
            for (var rulePos = 0; rulePos < post.length; rulePos++) {
                ...
                if (!flags(ruleByte, FLAG_ALPHA_OR_QUOT)) {
                    ...
                    if (!{
                            ...
                            '&': function () {
                                return flagsAt(text, ++pos, FLAG_DIPTHONG) || isOneOf(text.substr((++pos) - 2, 2), ['HC', 'HS']);
                            },
...
*/
    ...
    function Parser(input) {
        ...
        var getLength = function (pos) {
            return phonemeLength[pos] | 0;
        };
        ...
        return phonemeindex.map(function (v, i) {
            return [v, phonemeLength[i] | 0, stress[i] | 0];
        });
    }
...
// formant 2 frequencies (throat) 48..53
    ...
    function Renderer(phonemes, pitch, mouth, throat, speed, singmode) {
        ...
        var Output = new CreateOutputBuffer(
            441 // = (22050/50)
            * phonemes.reduce(function (pre, v) {
                return pre + (v[1] * 20);
            }, 0) / 50 // Combined phoneme length in ms.
